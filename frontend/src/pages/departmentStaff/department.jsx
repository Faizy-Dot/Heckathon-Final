...
const StaffForm = () => {
  ...
  const fetchBeneficiaryInfo = async () => {
    ...
    setError(""); // Reset error before calling API
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Construct the payload only when we are ready to submit
      const payload = {
        userId: beneficiaryInfo?._id, // Ensure beneficiaryInfo has _id
        remarks,
        updateStatus: status,
      };
      ...
      if (response.ok) {
        setShowReceipt(true);
        Swal.fire("Assistance details updated successfully!");
        // Reset the state *after* the action is successful
        setToken("");
        setBeneficiaryInfo(null);
        setRemarks("");
        setStatus("In Progress");
